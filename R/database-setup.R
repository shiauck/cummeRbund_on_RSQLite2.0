...
loadRepTable<-function(repTableFile,
	...
	#Fix sample_names
	full$condition<-dbQuoteIdentifier(dbConn,as.character(full$condition),unique=FALSE)
...
loadGenes<-function(fpkmFile,
	...
	samples<-dbQuoteIdentifier(dbConn,samples,unique=FALSE)
	...
	#Adjust sample names with dbQuoteIdentifier
	genemelt$sample_name <- dbQuoteIdentifier(dbConn,as.vector(genemelt$sample_name),unique=FALSE)
	...
	if(file.exists(diffFile)){
		...
		if(dim(diff)[1]>0){
			#Adjust sample names with dbQuoteIdentifier
			diff$sample_1<-dbQuoteIdentifier(dbConn,as.vector(diff$sample_1),unique=FALSE)
			diff$sample_2<-dbQuoteIdentifier(dbConn,as.vector(diff$sample_2),unique=FALSE)
	...
	if(file.exists(countFile)){
		...
		if(dim(counts)[1]>0){
			...
			#Adjust sample names with dbQuoteIdentifier
			countmelt$sample_name <- dbQuoteIdentifier(dbConn,as.vector(countmelt$sample_name),unique=FALSE)
	...
	if(file.exists(replicateFile)){
		...
		if(dim(reps)[1]>0){
			...
			#Adjust sample names with dbQuoteIdentifier
			reps$condition <- dbQuoteIdentifier(dbConn,as.character(reps$condition),unique=FALSE)
...
loadIsoforms<-function(fpkmFile,
	...
	#Adjust sample names with dbQuoteIdentifier
	isoformmelt$sample_name <- dbQuoteIdentifier(dbConn,as.vector(isoformmelt$sample_name),unique=FALSE)
...
loadTSS<-function(fpkmFile,
	...
	if (nrow(tssTable)>0){
		...
		#Adjust sample names with dbQuoteIdentifier
		tssmelt$sample_name <- dbQuoteIdentifier(dbConn,as.vector(tssmelt$sample_name),unique=FALSE)
...
loadCDS<-function(fpkmFile,
	...
	if (nrow(cdsTable)>0){
		...
		#Adjust sample names with dbQuoteIdentifier
		cdsmelt$sample_name <- dbQuoteIdentifier(dbConn,as.vector(cdsmelt$sample_name),unique=FALSE)
...
bulk_insert <- function(dbConn,sql,bound.data)
{
	...
	#extract labels from sql
	#example: INSERT INTO replicates VALUES(:file, :condition, :replicate_num, :rep_name, :total_mass, :norm_mass, :internal_scale, :external_scale)

	sql_parts <- unlist(strsplit(sql, "[\\(\\)]"))

	#sql_parts[1]: INSERT INTO replicates VALUES
	#sql_parts[2]: :file, :condition, :replicate_num, :rep_name, :total_mass, :norm_mass, :internal_scale, :external_scale

	lab_col <- unlist(strsplit(sql_parts[2], ",\\s?"))

	#remove colon

	lab_col <- substr(lab_col, 2, 100)

	#sql_parts[2]: file, condition, replicate_num, rep_name, total_mass, norm_mass, internal_scale, external_scale

	#assemble sql command
	for(df_ind in 1:length(bound.data[,1])) {
		if(all(lab_col %in% "")) {
			sql_cmd = paste(sql_parts[1],
			                "(\"",
			                paste(as.matrix(bound.data[df_ind, ]), collapse="\",\""),
			                "\")",
			                sep="")
		} else {
			sql_cmd = paste(sql_parts[1],
			                "(\"",
			                paste(as.matrix(bound.data[df_ind, lab_col]), collapse="\",\""),
			                "\")",
			                sep="")
		}
		dbExecute(dbConn, sql_cmd)
	}
	dbCommit(dbConn)
}
